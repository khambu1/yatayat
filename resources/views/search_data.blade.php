...
<div class="accordion" id="accordion_search">
    <div class="accordion-item">
        <h2 class="accordion-header" id="heading_{{$data->id}}">
            <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_{{$data->id}}" aria-expanded="true" aria-controls="collapse_{{$data->id}}" onclick="clearSeat('<?= $id ?>');
            ...
            <div class="accordion-body">
                <div class="row" id="details">
                    <div class="col-md-8">
                        ...
                        <form action="{{ url('bookseat') }}" method="POST" id="bookingForm{{ $data->id }}">
                            @csrf
                            ...
                            @if(backpack_user())
                            <button class="btn btn-md btn-primary mt-2 booking-confirmation" data-id="{{ $data->id }}">Continue Booking</button>
                            @else
...
<script>
    $(document).ready(function() {
        ...
        $('.booking-confirmation').click(function(e) {
            e.preventDefault();

            totalSeat = $('#total_seat').val();
            id = $(this).data('id');

            if (totalSeat > 0) {
                $('#bookingForm' + id).submit();
            } else {
                swal({
                    title: "Ohh no!",
                    text: "Please select your seat first!",
                    icon: "warning",
                    button: "OK",
                });
            }
